...
def classify_vehicle_indonesian(image, bbox, initial_vtype, v_conf):
    ...
    if initial_vtype == "bus":
        # Luxury MPV / Minibus (Toyota Alphard, Vellfire, Serena, Voxy, Staria, HiAce)
        # di gerbang parkir Indonesia adalah mobil penumpang kategori MPV (bukan bus komersial)!
        if (p_minibus + p_mpv + p_wagon) >= 0.25 or p_minibus >= 0.25 or p_mpv >= 0.15:
            return 'car', 'MPV', round(max(p_minibus, p_mpv, p_wagon, 0.85), 3)
    ...
    # Mobil MPV keluarga maupun Luxury MPV (Alphard, Vellfire, Voxy, Serena, Innova, Avanza, Staria)
    # diklasifikasikan sebagai MPV (bukan Bus). Sinyal p_minibus pada mobil penumpang diserap ke MPV!
    score_mpv = p_mpv * 1.6 + p_wagon * 1.2 + p_minibus * 1.5
